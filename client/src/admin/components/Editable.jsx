...
export const EditableText = ({
  ...
  return (
    <Tag
      ...
      title="Click to edit"
    >
      {value || <span className="opacity-25 italic font-normal text-sm">{placeholder}</span>}
    </Tag>
  );
};
...
export const EditableImage = ({ src, alt = '', onChange, className = '' }) => {
  ...
  const fallback = 'https://images.unsplash.com/photo-1509391366360-2e959784a276?w=400';
    ...
    <div className="relative group/ei">
      ...
      {editing && (
        <div className="absolute inset-0 bg-black/75 backdrop-blur-sm flex flex-col items-center justify-center z-20 p-5 rounded">
          <p className="text-white/70 text-[10px] uppercase tracking-widest font-bold mb-3">Paste image URL</p>
          <input
            autoFocus
            type="text"
            value={draft}
            ...
            className="w-full px-3 py-2.5 rounded-lg text-sm mb-3 bg-white text-gray-900 placeholder-gray-400 border-0 outline-none shadow-lg"
            placeholder="https://images.unsplash.com/…"
          />
          <div className="flex gap-2">
            <button onClick={commit} className="bg-amber-400 hover:bg-amber-300 text-black px-5 py-2 rounded-lg text-xs font-black transition-all">Apply</button>
            <button onClick={() => setEditing(false)} className="bg-white/20 hover:bg-white/30 text-white px-5 py-2 rounded-lg text-xs font-medium transition-all">Cancel</button>
          </div>
        </div>
      )}
    </div>
