...
const HomePageEditor = ({ token, onHasChanges, onSaveRef }) => {
  ...
  const getSection = (type) => sections.find((s) => s.section_type === type || s.type === type) || { content: {} };

  if (loading) return <div className="p-20 text-center font-black italic text-stone-400 uppercase tracking-widest">Opening Home Studio...</div>;
  ...
  return (
    <div className="min-h-screen overflow-auto bg-white pb-20">
      {/* SECTION 1: HERO */}
      <section className="min-h-[85vh] flex items-center justify-center px-6 py-10 border-b border-stone-100">
        <div className="w-full max-w-screen-2xl mx-auto flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-[0.9] lg:pr-12">
            ...
            <textarea 
              ...
              className="text-[#1A1102] text-5xl xl:text-7xl font-bold leading-tight mb-8 bg-transparent hover:bg-stone-50 outline-none w-full resize-none overflow-hidden"
              rows={2}
            />
            <textarea 
              value={hero.content.main_support_text || ''}
              onChange={(e) => update('hero', 'main_support_text', e.target.value)}
              className="text-[#606060] text-lg xl:text-xl font-medium max-w-xl mb-10 leading-relaxed bg-transparent hover:bg-stone-50 outline-none w-full resize-none overflow-hidden"
              rows={3}
            />
            <div className="flex items-center gap-4 mb-10">
                <button className="bg-[#1A1102] px-8 py-4 text-white font-bold rounded-md opacity-50 cursor-not-allowed">View our packages</button>
                <img src={heroFlower} className="w-8 h-8" alt="decor" />
            </div>
            <textarea 
              value={hero.content.second_support_text || ''}
              onChange={(e) => update('hero', 'second_support_text', e.target.value)}
              className="font-light text-[#606060] text-[17px] max-w-[260px] bg-transparent hover:bg-stone-50 outline-none w-full resize-none"
            />
          </div>

          {/* Hero Image Editor */}
          <div className="flex-[1.1] w-full aspect-[4/3] bg-stone-100 rounded-sm overflow-hidden relative group">
             <div className="absolute inset-0 flex flex-col items-center justify-center p-10 text-center">
                <p className="text-stone-400 font-bold mb-4 uppercase text-xs tracking-widest">Hero Image ID</p>
                <input 
                  value={hero.content.image_url || 'solar-hero-01'}
                  onChange={(e) => update('hero', 'image_url', e.target.value)}
                  className="bg-white border border-stone-200 px-4 py-2 rounded-lg font-mono text-xs w-full max-w-xs text-center outline-none focus:border-amber-400"
                />
             </div>
          </div>
        </div>
      ...
      <div className="w-full border-b border-stone-100 flex flex-col md:flex-row bg-white">
        <div className="flex-1 p-8 lg:p-12 border-r border-stone-100">
           <div className="flex items-center gap-4 mb-2">
              <input 
                value={hero.content.question_text || ''}
                onChange={(e) => update('hero', 'question_text', e.target.value)}
                className="text-xl font-bold uppercase tracking-tighter bg-transparent hover:bg-stone-50 outline-none"
              />
           ...
           <input 
                ...
                className="text-gray-500 font-medium bg-transparent hover:bg-stone-50 outline-none w-full"
            />
        </div>
        <div className="flex-1 p-8 lg:p-12 flex items-center justify-between">
           <div>
              <span className="text-sm font-bold text-gray-900 mb-2 block uppercase">Live Preview Amount</span>
              <h4 className="text-4xl font-bold">₦125,000</h4>
           </div>
           <div className="text-stone-300 italic text-[10px]">Logic is static</div>
        </div>
      ...
      <section className="container mx-auto flex flex-col lg:flex-row py-20 border-b border-stone-100">
         <div className="lg:w-1/3 p-8 border-r border-stone-100">
            <input 
            ...
            <textarea 
              ...
              className="text-4xl font-bold text-gray-900 mb-6 bg-transparent outline-none w-full resize-none"
            />
            <textarea 
         ...
         <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2">
            {stats.content.stats?.map((stat, i) => (
              <div key={i} className="p-10 border-b border-stone-100 hover:bg-stone-50/50 transition-colors">
                  <input 
                    value={stat.value}
                    onChange={(e) => {
                  ...
                  <input 
                    ...
                    className="text-lg font-semibold text-gray-900 mb-2 block bg-transparent outline-none w-full"
                  />
                  <textarea 
      ...
      <section className="bg-white my-12 border-y border-stone-100">
        <div className="container mx-auto flex flex-col lg:flex-row min-h-[500px]">
          <div className="lg:w-1/3 p-8 lg:p-16 border-r border-stone-100">
            <input 
            ...
            <textarea 
              ...
              onChange={(e) => update('blog-teaser', 'sub_heading', e.target.value)}
              className="text-gray-600 leading-relaxed bg-transparent outline-none w-full mb-8"
            />
          </div>
          <div className="lg:w-2/3 bg-stone-50 flex items-center justify-center italic text-stone-300">
        ...
        <div className="container mx-auto flex flex-col lg:flex-row">
          ...
          <div className="lg:w-2/3">
            {faqTeaser.content.faqs?.map((faq, i) => (
              <div key={i} className="border-b border-stone-100 p-8 lg:p-12 hover:bg-stone-50/30 transition-all">
                <input 
                  value={faq.question}
                  onChange={(e) => {
                  ...
                  className="text-xl font-bold text-stone-800 bg-transparent outline-none w-full mb-4"
                />
                <textarea 
                  value={faq.answer}
                  onChange={(e) => {
                  ...
                  className="text-stone-500 text-base leading-relaxed bg-transparent outline-none w-full resize-none"
                />
              </div>
            ))}
          </div>
      ...
      <section className="bg-white py-20">
        <div className="container mx-auto flex flex-col lg:flex-row border border-stone-100 rounded-[3rem] overflow-hidden">
          <div className="lg:w-2/3 bg-stone-50 flex flex-col items-center justify-center p-12">
             <p className="text-stone-400 font-black text-[10px] uppercase mb-2">CTA Image Asset ID</p>
             <input 
                value={cta.content.image_url || ''}
                onChange={(e) => update('cta', 'image_url', e.target.value)}
                className="bg-white border border-stone-200 px-4 py-2 rounded-lg font-mono text-xs w-64 text-center outline-none"
             />
          </div>
          <div className="lg:w-1/3 p-12 lg:p-20 flex flex-col justify-center">
            ...
            <input 
              ...
              className="text-4xl font-medium text-stone-400 bg-transparent outline-none"
            />
            <input 
              value={cta.content.heading_two || ''}
              onChange={(e) => update('cta', 'heading_two', e.target.value)}
              className="text-4xl font-bold text-stone-900 mb-10 bg-transparent outline-none"
            />
          </div>
        </div>
      </section>
