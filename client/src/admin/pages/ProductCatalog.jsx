...
const inputCls =
  "w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray  focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent focus:bg-white transition placeholder-gray-400";
...
const ImageField = ({
  ...
  const imgSrc =
    mode === "file" && file ? URL.createObjectURL(file) : value || preview;
      ...
      {imgSrc && (
        <div className="mt-2 relative inline-block">
          ...
          <button
            ...
            className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-gray  text-white rounded-full flex items-center justify-center hover:bg-red-600 transition"
          >
            <Icon name="x" size={10} />
          </button>
        </div>
...
const ProductCatalogPage = ({ token }) => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      setProducts(data);
      originalMap.current = Object.fromEntries(
    ...
    setProducts((prev) =>
      prev.map((p) => {
        ...
        setDirtyIds((prev) => {
          ...
          JSON.stringify(updated) !== originalMap.current[id]
            ? next.add(id)
            : next.delete(id);
          return next;
        });
  ...
  const saveSingle = async (product) => {
    ...
    if (product._imageFile)
      formData.append("featuredImage", product._imageFile);
    else if (product.featured_image_url)
      formData.append("featured_image_url", product.featured_image_url);
  ...
  const filtered = products.filter((p) => {
    ...
    if (
      filterStatus !== "all" &&
      getComputedStatus(p.stock, p.stock_status) !== filterStatus
    )
      return false;
  ...
  const stats = {
    ...
    inStock: products.filter(
      (p) => getComputedStatus(p.stock, p.stock_status) === "in_stock",
    ).length,
    lowStock: products.filter(
      (p) => getComputedStatus(p.stock, p.stock_status) === "low_stock",
    ).length,
    outOfStock: products.filter(
      (p) => getComputedStatus(p.stock, p.stock_status) === "out_of_stock",
    ).length,
  };
    ...
    >
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray  tracking-tight">
              Product Catalog
            </h1>
            <p className="text-sm text-gray-500 mt-0.5">
              {products.length} products · {dirtyIds.size} unsaved changes
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              ...
              className="rounded-2xl p-4 border border-white shadow-sm"
            >
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {s.label}
              </p>
              <p style={{ color: s.color }} className="text-3xl font-bold mt-1">
                {s.value}
              </p>
            </div>
          ))}
            ...
            <>
              ...
              {filtered.map((product) => (
                <ProductRow
                  ...
                  onUpdate={(updates) =>
                    handleInlineUpdate(product.id, updates)
                  }
...
const ProductRow = ({
  ...
  return (
    <div
      className={`border-b border-gray-50 last:border-0 transition-all ${isDirty ? "bg-amber-50/40" : "hover:bg-gray-50/60"}`}
    >
      {/* Summary row */}
      <div className="grid grid-cols-12 gap-3 px-5 py-3.5 items-center">
        ...
        <div className="col-span-4 flex items-center gap-3 min-w-0">
          <div className="w-11 h-11 flex-shrink-0 rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
            <img
              src={
                product.featured_image_url || "https://placehold.co/44?text=?"
              }
              alt={product.name}
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <p className="text-sm font-semibold text-gray  truncate">
                {product.name}
              </p>
              {product.is_featured && (
                <Icon
        ...
        <div className="col-span-2 flex justify-center">
          <StatusBadge
            stock={product.stock}
            manualStatus={product.stock_status}
          />
        </div>
        ...
        <div className="col-span-1 flex justify-center">
          <button
            ...
            title={
              product.is_visible ? "Visible to customers" : "Hidden from store"
            }
          >
...
const ProductDrawer = ({ isOpen, product, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setForm(
        product ? { ...BLANK_FORM, ...product, _imageFile: null } : BLANK_FORM,
      );
    }
  }, [isOpen, product]);
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed inset-y-0 right-0 w-full max-w-xl z-50 bg-white shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {isEdit ? `ID: ${product?.id}` : "Fields marked * are required"}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <section>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Category">
                    <input
                      ...
                      className={inputCls}
                      placeholder="Batteries, Inverters…"
                    />
                  </Field>
                  <Field label="Brand">
                    <input
                ...
                <Field
                  label="Slug (URL)"
                  hint="Auto-generated from name. Must be unique."
                >
                  <input
                    type="text"
                    value={form.slug}
                    ...
                    className={inputCls}
                    placeholder="5kwh-lithium-battery"
                  />
                </Field>
            ...
            <section>
              ...
              <div className="space-y-4">
                <Field
                  ...
                  hint="Displayed in product cards (max 500 chars)"
                >
                  <textarea
                    rows={2}
                ...
                {[
                  {
                    ...
                    desc: "Customers can see and purchase this product",
                  },
                  {
                    key: "is_featured",
                    label: "Featured product",
                    desc: "Show in homepage carousel / featured section",
                  },
                ].map(({ key, label, desc }) => (
                  <label
                    key={key}
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/80 flex items-center justify-between">
          ...
          >
            {submitting ? (
              ...
              <>
                <Icon name={isEdit ? "save" : "plus"} size={15} />
