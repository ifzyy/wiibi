      ...
      <div className="min-h-[85vh] flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-screen-2xl mx-auto flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-[0.9] lg:pr-12">
            ...
            />
            <EditableText
              value={content.main_support_text}
              onChange={(v) => u("main_support_text", v)}
              as="p"
              multiline
              className="text-[#606060] text-lg xl:text-xl font-medium max-w-xl mb-10 leading-relaxed"
            />
            <div className="flex gap-4 mb-12">
              <button className="bg-[#1A1102] px-8 py-4 text-white font-bold rounded-md">
                View our packages
              </button>
            </div>
            <div className="text-xl mb-3">🌸</div>
            <EditableText
              value={content.second_support_text}
              onChange={(v) => u("second_support_text", v)}
              as="p"
              multiline
          ...
          <div className="flex-[1.1] w-full aspect-[4/3] rounded-sm overflow-hidden shadow-2xl">
            <EditableImage
              src={
                ...
                "https://images.unsplash.com/photo-1583345237708-add35a664d77?w=1600&auto=format&fit=crop&q=80"
              }
              alt="Hero"
              onChange={(v) => u("hero_image_url", v)}
      ...
      <div className="w-full border-t border-b border-gray-100 flex flex-col md:flex-row">
        <div className="flex-1 p-8 lg:p-12 flex flex-col justify-center border-b md:border-b-0 md:border-r border-gray-100">
          <EditableText
            ...
            onChange={(v) => u("question_text", v)}
            as="h3"
            className="text-xl font-bold uppercase tracking-tighter mb-2"
          />
          <EditableText
            value={content.confidence_text}
            onChange={(v) => u("confidence_text", v)}
            as="p"
        ...
        <div className="flex-1 p-8 lg:p-12 flex items-center justify-between">
          <div>
            <span className="text-sm font-bold text-gray  block mb-2">
              Solar Calculator
            </span>
      ...
      <div className="container mx-auto px-6">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <EditableText
              ...
              as="h2"
              className="text-3xl font-bold text-gray "
            />
          </div>
          <EditableText
            value={content.button_text}
            onChange={(v) => u("button_text", v)}
            as="span"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {(content.products || []).slice(0, 4).map((prod, i) => (
            <div
              key={i}
              className="group/card rounded-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow"
            >
              <div className="aspect-square overflow-hidden bg-gray-50">
                <EditableImage
                  src={prod.image_url}
                  alt={prod.name}
                  onChange={(v) => updProd(i, "image_url", v)}
              ...
              <div className="p-4">
                <EditableText
                  value={prod.name}
                  onChange={(v) => updProd(i, "name", v)}
                  as="h4"
                  className="font-semibold text-gray  mb-1 text-sm"
                />
                <EditableText
                  value={prod.price}
                  onChange={(v) => updProd(i, "price", v)}
                  as="p"
      ...
      <div className="container mx-auto flex flex-col lg:flex-row">
        <div className="lg:w-1/3 p-8 lg:p-16 flex flex-col border-b lg:border-b-0 lg:border-r border-gray-200">
          ...
          <EditableText
            ...
            as="h2"
            multiline
            className="text-4xl font-bold text-gray  mb-6"
          />
          <EditableText
            value={content.paragraph_text}
            onChange={(v) => u("paragraph_text", v)}
            as="p"
            multiline
          ...
          {(content.stats || []).map((stat, i) => (
            ...
            >
              ...
              <EditableText
                ...
                as="p"
                className="text-lg font-semibold text-gray  mb-4 capitalize"
              />
              <EditableText
                value={stat.description}
                onChange={(v) => updStat(i, "description", v)}
                as="p"
                multiline
      ...
      <div className="container mx-auto flex flex-col lg:flex-row min-h-[600px]">
        ...
        <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2">
          {(content.posts || []).slice(0, 2).map((post, i) => (
            <div
              key={i}
              className="relative group/post overflow-hidden border-r border-gray-200 last:border-r-0 min-h-[400px]"
            >
              <EditableImage
                src={
                  post.image_url ||
                ...
                alt={post.title}
                onChange={(v) => updPost(i, "image_url", v)}
              ...
              <div className="absolute bottom-12 left-10 right-10 text-white">
                ...
                <EditableText
                  value={post.excerpt}
                  onChange={(v) => updPost(i, "excerpt", v)}
                  as="p"
                  multiline
...
export const TestimonialsSection = ({ content, onChange }) => {
  ...
  return (
    <section className="bg-[#FFAA14 ] -50 py-16">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(content.testimonials || []).map((t, i) => (
            <div
              key={i}
              className="bg-white p-8 rounded-xl shadow-sm border  border-stone-50 -100"
            >
              <div className="text-amber-400 text-3xl font-serif mb-4 leading-none">
                "
              </div>
              <EditableText
                value={t.quote}
                onChange={(v) => updT(i, "quote", v)}
                as="p"
                multiline
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <EditableText
                    value={t.name}
                    onChange={(v) => updT(i, "name", v)}
                    as="p"
                    className="font-bold text-gray  text-sm"
                  />
                  <EditableText
                    value={t.role}
                    onChange={(v) => updT(i, "role", v)}
                    as="p"
      ...
      <div className="container mx-auto flex flex-col lg:flex-row">
        <div className="lg:w-1/3 p-8 lg:p-16 flex flex-col border-b lg:border-b-0 lg:border-r border-gray-200">
          ...
          <EditableText
            ...
            as="h2"
            multiline
            className="text-4xl font-bold text-gray  mb-6 leading-tight"
          />
          <EditableText
            value={content.sub_heading}
            onChange={(v) => u("sub_heading", v)}
        ...
        <div className="lg:w-2/3">
          {(content.faqs || []).map((faq, i) => (
            <details
              key={i}
              className="group/faq border-b border-gray-200 last:border-b-0"
            >
              <summary className="flex items-center justify-between p-8 lg:p-10 cursor-pointer list-none hover:bg-gray-50 transition-colors">
                <EditableText
                  value={faq.question}
                  onChange={(v) => updFaq(i, "question", v)}
                  as="span"
                  className="text-xl font-medium text-gray-800 pr-4"
                />
                <span className="text-2xl text-gray-400 group-open/faq:rotate-45 transition-transform duration-200 flex-shrink-0">
                  +
                </span>
              </summary>
      ...
      <div className="container mx-auto flex flex-col lg:flex-row min-h-[500px]">
        ...
        <div className="lg:w-1/3 p-12 lg:p-20 flex flex-col justify-center border-l border-gray-100">
          ...
          <EditableText
            ...
            onChange={(v) => u("heading_one", v)}
            as="h2"
            className="text-4xl md:text-5xl font-medium text-gray-500 mb-2"
          />
          <EditableText
            value={content.heading_two}
            onChange={(v) => u("heading_two", v)}
            as="h2"
            className="text-4xl md:text-5xl font-bold text-gray  mb-10"
          />
